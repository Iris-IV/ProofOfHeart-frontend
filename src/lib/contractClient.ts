 ...
 */
export async function getContributionXlm(
  campaignId: number,
  contributor: string
): Promise<number> {
  if (USE_MOCKS) {
  ...
  const stroops = await getContribution(campaignId, contributor);
 ...
 */
export async function claimRefundXlm(
  campaignId: number,
  contributor: string
  ...
  if (USE_MOCKS) {
    ...
    const key = `${campaignId}:${contributor}`;
    if (MOCK_REFUNDED.has(key)) throw new Error('Already refunded');
    const amount = MOCK_CONTRIBUTIONS[key] ?? 0;
    if (amount === 0) throw new Error('No contribution found for this campaign.');
  ...
  const txHash = await claimRefund(campaignId, contributor);
  return { transactionHash: txHash, refundedAmount: 0 };
}
