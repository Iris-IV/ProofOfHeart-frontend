...
export default function InstallFreighterModal({
  ...
  const mobile = useMemo(() => (isOpen ? isMobile() : false), [isOpen]);
      ...
      <div className="mx-4 w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-zinc-900">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-50">
            {socialLogin ? "Connect a wallet" : showMobileGuidance ? "Use a desktop browser" : "Freighter Wallet Required"}
          </h2>

          <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
            {showMobileGuidance
              ? "Freighter is a desktop browser extension and does not work on mobile. Please open this page on a desktop browser (Chrome, Edge, or Firefox) and install Freighter there."
              : socialLogin
          ...
          {!browser.supported && browser.name && !showMobileGuidance && (
