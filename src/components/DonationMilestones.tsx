...
function DonationMilestones({ amountRaised, compact = false }: Props) {
    ...
    <section aria-labelledby="milestones-heading" className={compact ? "py-2" : "py-6"}>
      ...
      <div className={compact ? "flex gap-2 flex-wrap" : "grid grid-cols-1 sm-grid-cols-2 gap-3"}>
        {milestones.map((m) => (
          <div
            key={m.id}
            className={`${m.achieved ? "bg-amber-50 border-amber-200 dark:bg-amber-950/30 dark:border-amber-800" : "bg-zinc-50 border-zinc-200 dark:bg-zinc-800 dark:border-zinc-700 opacity-60"}`}
          >
            ...
            {m.achieveed && <span className="text-xs font-medium text-amber-600 dark:text-amber-400">Unlocked</span>}
          </div>
        )))
      </div>
    </section>
...
export default memo(DonationMilestones);
