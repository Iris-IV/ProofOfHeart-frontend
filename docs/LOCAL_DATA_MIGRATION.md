...
| `stellar_wallet_public_key`                     | `WalletContext`          | Last connected wallet address | **Moved to `sessionStorage` (#1139).** One-time migration clears the legacy `localStorage` entry. This is a convenience cache, not source of truth.      |
